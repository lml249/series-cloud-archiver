...
class MV3WrongRootRepairTest(unittest.TestCase):
    ...
    def test_wrong_root_repair_direct_season_root_dry_run(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_urlopen(request, timeout):
                calls.append((request.get_method(), request.full_url, getattr(request, "data", None)))
                return _fake_mv3_direct_season_wrong_root_response(request, moved=False)
            ...
            self.assertTrue(report["ok"])
            self.assertTrue(report["dry_run"])
            self.assertEqual(report["wrong_root_title_count"], 0)
            self.assertEqual(report["wrong_root_direct_season_count"], 1)
            self.assertEqual(report["items"][0]["title"], title)
            self.assertEqual(report["items"][0]["season"], 7)
            self.assertEqual(report["items"][0]["correct_season_path"], f"/已整理/series/{title}/Season 07")
            self.assertEqual(report["items"][0]["decision"], "move_wrong_media_to_correct_season")
            self.assertEqual(report["items"][0]["action"], "dry_run_move_wrong_media_to_correct_season")
    ...
    def test_wrong_root_direct_season_pair_dry_run_plans_cloud_move_and_strm_rewrite(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            title = "广告狂人 (2007) {tmdbid=1104}"
            strm_title_root = Path(tmp) / "strm" / "series" / title
            season_dir = strm_title_root / "Season 07"
            ...
            original_targets = []
            for episode in (1, 2):
                content = f"/已整理/series/美剧【广告狂人】1-7季全 1080P中字【373G】/S07/E{episode:02d}.mkv"
                original_targets.append(content)
                (season_dir / f"广告狂人 - S07E{episode:02d}.strm").write_text(content, encoding="utf-8")

            calls = []

            def fake_urlopen(request, timeout):
                calls.append((request.get_method(), request.full_url, getattr(request, "data", None)))
                return _fake_mv3_direct_pair_repair_response(request, moved=False, target_created=False)
            ...
            with patch("urllib.request.urlopen", fake_urlopen):
                report = repair_mv3_wrong_root_direct_season_pair(
                    ...
                    expected_rewrite_count=2,
                )

            self.assertTrue(report["ok"])
            self.assertTrue(report["dry_run"])
            self.assertEqual(report["precheck"]["wrong"]["media_count"], 2)
            self.assertEqual(report["precheck"]["strm"]["wrong_target_count"], 2)
            self.assertEqual(report["precheck"]["rewrite_preview"]["summary"]["rewritable_count"], 2)
            self.assertFalse(any("/api/v1/files/115/move" in url for _method, url, _body in calls))
            self.assertEqual(
                [path.read_text(encoding="utf-8") for path in sorted(season_dir.glob("*.strm"))],
                original_targets,
            )

    def test_wrong_root_direct_season_pair_approval_moves_cloud_and_rewrites_strm(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            title = "广告狂人 (2007) {tmdbid=1104}"
            strm_title_root = Path(tmp) / "strm" / "series" / title
            season_dir = strm_title_root / "Season 07"
            ...
            for episode in (1, 2):
                (season_dir / f"广告狂人 - S07E{episode:02d}.strm").write_text(
                    f"/已整理/series/美剧【广告狂人】1-7季全 1080P中字【373G】/S07/E{episode:02d}.mkv",
                    encoding="utf-8",
                )
            posted = []
            state = {"moved": False, "target_created": False}
            ...
            def fake_urlopen(request, timeout):
                if getattr(request, "data", None):
                    ...
                    if request.full_url.endswith("/api/v1/files/115/folder"):
                        body = posted[-1][1]
                        if body.get("name") == "Season 07":
                            state["target_created"] = True
                    if request.full_url.endswith("/api/v1/files/115/move"):
                        state["moved"] = True
                return _fake_mv3_direct_pair_repair_response(request, moved=state["moved"], target_created=state["target_created"])
            ...
            self.assertEqual([operation["step"] for operation in report["operations"]], ["ensure_correct_season_path", "move_wrong_media_to_correct_season", "rewrite_strm_targets"])
            move_bodies = [body for url, body in posted if url.endswith("/api/v1/files/115/move")]
            self.assertEqual(move_bodies, [{"file_ids": ["wrong-s7-file-1", "wrong-s7-file-2"], "target_cid": "correct-season-7", "storage": "115-default"}])
            rewritten = [path.read_text(encoding="utf-8") for path in sorted(season_dir.glob("*.strm"))]
            self.assertTrue(all("/已整理/series/广告狂人 (2007) {tmdbid=1104}/Season 07/" in content for content in rewritten))
            self.assertTrue(all("美剧【广告狂人】" not in content for content in rewritten))
            self.assertEqual(report["post_verify"]["wrong"]["media_count"], 0)
            self.assertEqual(report["post_verify"]["correct"]["media_count"], 2)
            self.assertEqual(report["post_verify"]["strm"]["wrong_target_count"], 0)
            self.assertEqual(report["post_verify"]["strm"]["correct_target_count"], 2)

    def test_cli_writes_wrong_root_direct_season_pair_dry_run_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            env_file = tmp_path / ".env"
            output = tmp_path / "pair.json"
            title = "广告狂人 (2007) {tmdbid=1104}"
            strm_title_root = tmp_path / "strm" / "series" / title
            season_dir = strm_title_root / "Season 07"
            season_dir.mkdir(parents=True)
            for episode in (1, 2):
                (season_dir / f"广告狂人 - S07E{episode:02d}.strm").write_text(
            ...
            with patch("urllib.request.urlopen", lambda request, timeout: _fake_mv3_direct_pair_repair_response(request, moved=False, target_created=False)):
                code = main(
                    [
    ...
    def test_wrong_root_direct_season_pair_supports_specials_season_zero(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            wrong_title = "Band of Brothers {tmdbid=4613}"
            correct_title = "兄弟连 (2001) {tmdbid=4613}"
            strm_title_root = Path(tmp) / "strm" / "未识别" / wrong_title
            season_dir = strm_title_root / "Season 0"
            season_dir.mkdir(parents=True)
            for episode in (1, 2):
                target = f"/已整理/未识别/{wrong_title}/Season 0/Band of Brothers - S00E{episode:02d}.mkv"
                (season_dir / f"Band of Brothers - S00E{episode:02d}.strm").write_text(target, encoding="utf-8")

            with patch("urllib.request.urlopen", lambda request, timeout: _fake_mv3_direct_pair_season0_response(request)):
                report = repair_mv3_wrong_root_direct_season_pair(
                    "http://mv3.example",
                    "token",
                    f"/已整理/未识别/{wrong_title}",
                    f"/已整理/series/{correct_title}",
                    str(strm_title_root),
                    season=0,
                    storage="115-default",
                    title_filter=correct_title,
                    expected_episode_count=2,
                    expected_episode_min=1,
                    expected_episode_max=2,
                    expected_rewrite_count=2,
            ...
            self.assertTrue(report["ok"], report["blockers"])
            self.assertTrue(report["dry_run"])
            self.assertEqual(report["wrong_season_path"], f"/已整理/未识别/{wrong_title}/Season 0")
            self.assertEqual(report["correct_title_path"], f"/已整理/series/{correct_title}")
            self.assertEqual(report["correct_season_path"], f"/已整理/series/{correct_title}/Season 00")
            self.assertEqual(report["precheck"]["wrong"]["media_count"], 2)
            self.assertEqual(report["precheck"]["strm"]["wrong_target_count"], 2)
            self.assertEqual(report["precheck"]["rewrite_preview"]["summary"]["rewritable_count"], 2)

    def test_wrong_root_direct_season_pair_specials_approval_moves_and_rewrites(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            wrong_title = "Band of Brothers {tmdbid=4613}"
            correct_title = "兄弟连 (2001) {tmdbid=4613}"
            strm_title_root = Path(tmp) / "strm" / "未识别" / wrong_title
            ...
            season_dir.mkdir(parents=True)
            for episode in (1, 2):
                (season_dir / f"Band of Brothers - S00E{episode:02d}.strm").write_text(
                    f"/已整理/未识别/{wrong_title}/Season 0/Band of Brothers - S00E{episode:02d}.mkv",
                    encoding="utf-8",
                )
            posted = []
            state = {"moved": False, "target_created": False}
            ...
            def fake_urlopen(request, timeout):
                if getattr(request, "data", None):
                    ...
                    if request.full_url.endswith("/api/v1/files/115/folder"):
                        body = posted[-1][1]
                        if body.get("name") == "Season 00":
                            state["target_created"] = True
                    if request.full_url.endswith("/api/v1/files/115/move"):
                        state["moved"] = True
                return _fake_mv3_direct_pair_season0_response(
                    request,
                    moved=state["moved"],
                    target_created=state["target_created"],
                )
            ...
            self.assertTrue(report["write_executed"])
            move_bodies = [body for url, body in posted if url.endswith("/api/v1/files/115/move")]
            self.assertEqual(move_bodies, [{"file_ids": ["wrong-s0-file-1", "wrong-s0-file-2"], "target_cid": "correct-season-0", "storage": "115-default"}])
            rewritten = [path.read_text(encoding="utf-8") for path in sorted(season_dir.glob("*.strm"))]
            self.assertTrue(all(f"/已整理/series/{correct_title}/Season 00/" in content for content in rewritten))
            self.assertTrue(all("/已整理/未识别/" not in content for content in rewritten))
            self.assertEqual(report["post_verify"]["wrong"]["media_count"], 0)
            self.assertEqual(report["post_verify"]["correct"]["media_count"], 2)
            self.assertEqual(report["post_verify"]["strm"]["wrong_target_count"], 0)
            self.assertEqual(report["post_verify"]["strm"]["correct_target_count"], 2)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_urlopen(request, timeout):
                if getattr(request, "data", None):
                    ...
                    if request.full_url.endswith("/api/v1/files/115/delete"):
                        state["deleted"] = True
                return _fake_mv3_direct_season_wrong_root_response(request, moved=state["moved"], deleted=state["deleted"])
            ...
            self.assertEqual(move_bodies, [{"file_ids": ["wrong-s7-file-1", "wrong-s7-file-2"], "target_cid": "correct-season-7", "storage": "115-default"}])
            self.assertEqual(report["items"][0]["decision"], "move_wrong_media_to_correct_season")
            self.assertEqual(report["items"][0]["post_verify"]["correct"]["media_count"], 2)
...
def _fake_mv3_direct_season_wrong_root_response(request, moved=False, deleted=False):
    class FakeResponse:
        status = 200
    ...
    browse = {
        "wrong-root-direct": [] if deleted else [{"name": "S07", "cid": "wrong-season-7", "is_dir": True}],
...
def _fake_mv3_direct_pair_repair_response(request, moved=False, target_created=False):
    class FakeResponse:
        status = 200
    ...
    if parsed.path.endswith("/api/v1/files/115/folder"):
        ...
        if body.get("name") == "Season 07":
            return FakeResponse({"success": True, "data": {"cid": "correct-season-7"}})
        return FakeResponse({"success": True, "data": {"cid": f"created-{body.get('name', 'folder')}"}})
    if parsed.path.endswith("/api/v1/files/115/move"):
        return FakeResponse({"success": True, "message": "ok", "data": None})
    ...
    wrong_root = "/已整理/series/美剧【广告狂人】1-7季全 1080P中字【373G】"
    correct_title = f"/已整理/series/{title}"
    correct_season = f"{correct_title}/Season 07"
    wrong_files = [
        {"name": "Mad.Men.S07E01.mkv", "fid": "wrong-s7-file-1", "is_dir": False},
        {"name": "Mad.Men.S07E02.mkv", "fid": "wrong-s7-file-2", "is_dir": False},
    ]
    ...
    info = {
        "/已整理": {"name": "已整理", "cid": "organized-root", "is_dir": True},
        "/已整理/series": {"name": "series", "cid": "series-root", "is_dir": True},
        wrong_root: {"name": "美剧【广告狂人】1-7季全 1080P中字【373G】", "cid": "wrong-root-direct", "is_dir": True},
        f"{wrong_root}/S07": {"name": "S07", "cid": "wrong-season-7", "is_dir": True},
        correct_title: {"name": title, "cid": "correct-title-madmen", "is_dir": True},
    }
    if target_created or moved:
        info[correct_season] = {"name": "Season 07", "cid": "correct-season-7", "is_dir": True}
    browse = {
        "0": [{"name": "已整理", "cid": "organized-root", "is_dir": True}],
        "organized-root": [{"name": "series", "cid": "series-root", "is_dir": True}],
        "series-root": [
            {"name": "美剧【广告狂人】1-7季全 1080P中字【373G】", "cid": "wrong-root-direct", "is_dir": True},
            {"name": title, "cid": "correct-title-madmen", "is_dir": True},
        ],
        "wrong-root-direct": [{"name": "S07", "cid": "wrong-season-7", "is_dir": True}],
...
def _fake_mv3_direct_pair_season0_response(request, moved=False, target_created=False):
    class FakeResponse:
        status = 200
    ...
    if parsed.path.endswith("/api/v1/files/115/folder"):
        body = json.loads(request.data.decode("utf-8"))
        if body.get("name") == "Season 00":
            return FakeResponse({"success": True, "data": {"cid": "correct-season-0"}})
        return FakeResponse({"success": True, "data": {"cid": f"created-{body.get('name', 'folder')}"}})
    if parsed.path.endswith("/api/v1/files/115/move"):
        return FakeResponse({"success": True, "message": "ok", "data": None})
    ...
    wrong_title = "Band of Brothers {tmdbid=4613}"
    correct_title = "兄弟连 (2001) {tmdbid=4613}"
    wrong_root = f"/已整理/未识别/{wrong_title}"
    wrong_season = f"{wrong_root}/Season 0"
    correct_title_path = f"/已整理/series/{correct_title}"
    correct_season = f"{correct_title_path}/Season 00"
    wrong_files = [
        {"name": "Band of Brothers - S00E01.mkv", "fid": "wrong-s0-file-1", "is_dir": False},
        {"name": "Band of Brothers - S00E02.mkv", "fid": "wrong-s0-file-2", "is_dir": False},
    ]
    correct_files = [
        {"name": "Band of Brothers - S00E01.mkv", "fid": "wrong-s0-file-1", "is_dir": False},
        {"name": "Band of Brothers - S00E02.mkv", "fid": "wrong-s0-file-2", "is_dir": False},
    ...
    info = {
        "/已整理": {"name": "已整理", "cid": "organized-root", "is_dir": True},
        "/已整理/未识别": {"name": "未识别", "cid": "unrecognized-root", "is_dir": True},
        "/已整理/series": {"name": "series", "cid": "series-root", "is_dir": True},
        wrong_root: {"name": wrong_title, "cid": "wrong-title-brothers", "is_dir": True},
        wrong_season: {"name": "Season 0", "cid": "wrong-season-0", "is_dir": True},
        correct_title_path: {"name": correct_title, "cid": "correct-title-brothers", "is_dir": True},
    }
    if target_created or moved:
        info[correct_season] = {"name": "Season 00", "cid": "correct-season-0", "is_dir": True}
    browse = {
        "unrecognized-root": [{"name": wrong_title, "cid": "wrong-title-brothers", "is_dir": True}],
        "wrong-title-brothers": [{"name": "Season 0", "cid": "wrong-season-0", "is_dir": True}],
        "wrong-season-0": [] if moved else wrong_files,
        "series-root": [{"name": correct_title, "cid": "correct-title-brothers", "is_dir": True}],
        "correct-title-brothers": [{"name": "Season 00", "cid": "correct-season-0", "is_dir": True}] if target_created or moved else [],
        "correct-season-0": correct_files if moved else [],
    }
...
class MV3ProbeTest(unittest.TestCase):
    ...
    def test_ensure_115_path_resolves_created_folder_when_api_returns_null_data(self) -> None:
        calls = []
        folders = {
            "0": [],
            "10": [],
        ...
        def fake_urlopen(request, timeout):
            ...
            body = json.loads(request.data.decode("utf-8"))
            if body["name"] == "未整理":
                folders["0"].append({"n": "未整理", "cid": "10"})
            elif body["name"] == "Season 01":
                folders["10"].append({"n": "Season 01", "cid": "20"})
        ...
        self.assertEqual([step["resolved_by"] for step in report["steps"]], ["post_create_browse", "post_create_browse"])
        self.assertEqual(sum(1 for method, _url, _body in calls if method == "POST"), 2)
    ...
    def test_cloud_index_plan_matches_root_folders_by_tmdb_and_title(self) -> None:
            ...
            "items": [
                {
                    ...
                    "expected_count": 12,
                    "source_paths": ["/example/繁城之下 (2023) {tmdbid=233959}"],
                    "search_keywords": ["Ripe Town"],
                },
                {"title": "庆余年", "tmdbid": 93740, "season": 1, "expected_count": 46},
            ],
        }
                ...
                return json.dumps(
                    {
                        ...
                        "data": {
                            "items": [
                                {"name": "繁城之下 (2023) {tmdbid=233959}", "cid": "folder-1", "is_dir": True},
                                {"name": "庆余年1-2", "cid": "folder-2", "is_dir": True},
                                {"name": "别的剧", "cid": "folder-3", "is_dir": True},
                            ]
                        },
                    }
                ).encode("utf-8")
        ...
        rendered = render_mv3_cloud_index_plan_report(report, "json")
        self.assertEqual(report["mode"], "readonly-mv3-cloud-index-plan")
        self.assertEqual(report["root_folder_count"], 3)
        self.assertEqual(report["items_with_matches"], 2)
        self.assertEqual(report["items"][0]["best_match"]["file_id"], "folder-1")
        self.assertIn("tmdbid_match", report["items"][0]["best_match"]["reasons"])
        self.assertEqual(report["items"][1]["best_match"]["file_id"], "folder-2")
        self.assertIn("title_contains_match", report["items"][1]["best_match"]["reasons"])
        self.assertIn("STRM library side", report["safety"])
        self.assertNotIn("pickcode", rendered.lower())

    def test_cli_writes_cloud_index_plan_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            env_file = tmp_path / ".env"
            transfer_plan = tmp_path / "transfer.json"
            ...
            class FakeResponse:
                ...
                def read(self, _limit=-1):
                    return json.dumps({"success": True, "data": {"items": [{"name": "Demo", "cid": "folder-1", "is_dir": True}]}}).encode("utf-8")
    ...
    def test_cloud_sidecar_scan_recurses_into_115_fid_fc_zero_folders(self) -> None:
        class FakeResponse:
            status = 200
                    ...
                    return FakeResponse(
                        {
                            ...
                            "data": {
                                "items": [
                                    {"fn": "Demo (2026) {tmdbid=123}", "fid": "title-id", "pid": "root-id", "fc": "0"},
                                ]
                            },
                        }
                    )
                    ...
                    return FakeResponse(
                        {
                            ...
                            "data": {
                                "items": [
                                    {"fn": "Season 1", "fid": "season-id", "pid": "title-id", "fc": "0"},
                                ]
                            },
                        }
                    )
                    ...
                    return FakeResponse(
                        {
                            ...
                            "data": {
                                "items": [
                                    {"fn": "Demo.S01E01.mkv", "fid": "video-1", "pid": "season-id", "s": 1000, "sha1": "abc"},
                                    {"fn": "Demo.S01E01.nfo", "fid": "nfo-1", "pid": "season-id", "s": 10, "sha1": "def"},
                                ]
                            },
                        }
                    )
    ...
    def test_share_preview_requests_full_folder_and_keeps_more_than_50_items(self) -> None:
        ...
        def fake_urlopen(request, timeout):
            ...
            seen.append((path, body))
            if path == "/api/v1/resource-search/search":
                return FakeResponse({"success": True, "data": {"items": [{"title": "甄嬛传", "share_link": "https://example.test/s/private"}]}})
            if path == "/api/v1/share-transfer/parse":
                return FakeResponse({"success": True, "data": {"share_code": "parsed-code"}})
            if path == "/api/v1/share-transfer/browse":
                self.assertEqual(body["storage"], "115-default")
        ...
        with patch("urllib.request.urlopen", fake_urlopen):
            report = preview_mv3_share(
                ...
                "甄嬛传",
                selection_index=1,
                expected_title_contains="甄嬛传",
            )

        self.assertEqual([item[0] for item in seen], ["/api/v1/resource-search/search", "/api/v1/share-transfer/parse", "/api/v1/share-transfer/browse"])
        self.assertTrue(report["ok"])
        self.assertEqual(report["browse_limit"], 1150)
        self.assertEqual(report["browse"]["item_count"], 76)
        self.assertEqual(len(report["browse"]["items"]), 76)
        self.assertEqual(report["browse"]["items"][-1]["episode"], 76)
    ...
    def test_share_receive_selected_folder_requires_verified_nested_preview(self) -> None:
        seen = []
        ...
        def fake_urlopen(request, timeout):
            ...
            seen.append((path, body))
            if path == "/api/v1/resource-search/search":
                return FakeResponse({"success": True, "data": {"items": [{"title": "唐诡奇谭之长安县尉", "share_link": "https://example.test/s/private"}]}})
            if path == "/api/v1/share-transfer/parse":
                return FakeResponse({"success": True, "data": {"share_code": "parsed-code", "receive_code": "abcd"}})
            if path == "/api/v1/share-transfer/browse":
                return FakeResponse({"success": True, "data": {"items": [{"name": "唐诡奇谭之长安县尉[全56集]", "cid": "folder-1", "is_dir": True}]}})
            if path == "/api/v1/share-transfer/receive":
                raise AssertionError("folder receive must be blocked without a verified nested preview")
            raise AssertionError(f"unexpected path: {path}")
        ...
        with patch("urllib.request.urlopen", fake_urlopen):
            report = receive_mv3_share(
                ...
                selection_index=1,
                browse_index=1,
                receive_selected_folder=True,
                expected_episode_count=56,
                expected_episode_min=1,
                expected_episode_max=56,
                expected_title_contains="唐诡",
                target_path="/未整理",
                storage="115-default",
            )
        ...
        self.assertFalse(report["ok"])
        self.assertEqual(report["file_id_count"], 0)
        self.assertIn("verified_folder_browse_report_required", report["warnings"])
        self.assertNotIn("/api/v1/share-transfer/receive", [item[0] for item in seen])

    def test_share_receive_selected_folder_uses_verified_folder_id(self) -> None:
        seen = []
        ...
        def fake_urlopen(request, timeout):
            ...
            if path == "/api/v1/share-transfer/browse":
                return FakeResponse({"success": True, "data": {"items": [{"name": "唐诡奇谭之长安县尉[全56集]", "cid": "folder-1", "is_dir": True}]}})
            if path == "/api/v1/share-transfer/receive":
                self.assertEqual(body["file_ids"], ["folder-1"])
                return FakeResponse({"success": True, "data": {"record_id": "record-1", "recv_folder_count": 1}})
            raise AssertionError(f"unexpected path: {path}")
        ...
        with patch("urllib.request.urlopen", fake_urlopen):
            report = receive_mv3_share(
                ...
                browse_index=1,
                receive_selected_folder=True,
                verified_folder_browse_report=verified_report,
                expected_episode_count=56,
                expected_episode_min=1,
                expected_episode_max=56,
                expected_title_contains="唐诡",
        ...
        self.assertTrue(report["ok"])
        self.assertEqual(report["file_id_count"], 1)
        self.assertEqual(report["selected_item_count"], 1)
        self.assertTrue(report["receive_selected_folder"])
        self.assertEqual(report["browse_selection"]["name"], "唐诡奇谭之长安县尉[全56集]")
        self.assertEqual(seen[-1][1]["file_ids"], ["folder-1"])
    ...
    def test_organize_transfer_from_browse_report_posts_complete_file_list(self) -> None:
        ...
        self.assertIn(
            ...
            report["completion_verification"]["required_followup"],
        )
        self.assertNotIn("token", rendered)
    ...
    def test_cli_share_receive_selected_folder_uses_verified_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            env_file = tmp_path / ".env"
            ...
            payload = json.loads(output.read_text(encoding="utf-8"))
            self.assertTrue(payload["ok"])
            self.assertTrue(payload["receive_selected_folder"])
            self.assertEqual(seen[-1][1]["file_ids"], ["folder-1"])
